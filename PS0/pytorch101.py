...
def mutate_tensor(
    ...
    index = 0
    for i in indices:
      if (i[0] >= x.shape[0] or i[1] >= x.shape[1]):
        assert("invalid mutate!")
      x[i[0], i[1]] = values[index]
      index += 1
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx0 = [i for i in range(x.shape[0])]
    idx0 = idx0[::-1]
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    y = y.reshape(2, 3, 4).transpose(1, 2).reshape(8, 3).t()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mean_by_col = torch.sum(y, dim = 0) / y.shape[0]
    mean_by_col = mean_by_col.reshape(1, -1) # 1 by N vector dim: 2
   
    y_without_mean_squared = (y.clone() - mean_by_col) ** 2
    deviation_by_col = torch.sqrt(torch.sum(y_without_mean_squared, dim = 0) / (y.shape[0] - 1)).reshape(1, -1)
    
    y = (y - mean_by_col) / deviation_by_col
