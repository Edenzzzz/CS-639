...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        train_vec = x_train[i].clone().reshape(-1, 1) # dim: 3072 * 1
        test_vec = x_test[j].clone().reshape(-1, 1) # dim: 3072 * 1
        vec_diff_square = (train_vec - test_vec) ** 2
        l2_norm_square = vec_diff_square.sum(dim = 0)
        l2_norm = l2_norm_square.sqrt()
        dists[i, j] = l2_norm
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # functions from torch.nn or torch.nn.functional.                        #
    ##########################################################################
    x_test_reshaped = x_test.clone().reshape(num_test, -1)
    for i in range(num_train):
      reshaped_x_train_i = x_train[i].clone().reshape(1, -1)
      l2_norm_squared = ((reshaped_x_train_i - x_test_reshaped) ** 2).sum(dim = 1)
      l2_norm = (l2_norm_squared.sqrt()).t()
      dists[i] = l2_norm
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Use (a - b)^2 = a^2 + b^2 - 2ab to vectorize
    x_train_flattened = x_train.reshape(num_train, -1)
    x_test_flattened = x_test.reshape(num_test, -1)
    x_train_squared = (x_train_flattened ** 2).sum(dim = 1, keepdim = True)
    x_test_squared_T = (x_test_flattened ** 2).sum(dim = 1, keepdim = True).t()
    
    # x_train will be broadcast from (100, 1) to (100, 100)
    # y_train will be broadcast from (1, 100) to (100, 100)
    dists = (x_train_squared + x_test_squared_T -  x_train_flattened.mm(x_test_flattened.t()) * 2).sqrt()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    results = []
    for j in range(num_test):
      dist_vect = dists[:, j].clone()
      max_val = dist_vect.max()
      ind = []
      # find the k-min scores' indices
      for i in range(k):
          _, index = dist_vect.min(dim = 0)
          ind.append(index.item())
          dist_vect[index] = max_val

      # transfer the indices to the associated class labels
      result = []
      for i in ind:
          result.append(y_train[i].item())
      
      # find the most common class label and handle the ties.
      freq = 0
      num = result[0]
      for i in result:
    ...
    y_pred = torch.tensor(results).to(torch.int64)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        y_test_pred = predict_labels(dist, self.y_train, k = k)
...
def knn_cross_validate(
    ...
    if (len(x_train_folds) != num_folds or len(y_train_folds) != num_folds):
    ...
    for k in k_choices:
      accuracies = []
      for i in list(range(num_folds)):
        validation_data = x_train_folds[i]
        validation_label = y_train_folds[i]
        remaining_ind = [j for j in range(num_folds) if j != i]
        training_data = x_train_folds[remaining_ind[0]]
        training_label = y_train_folds[remaining_ind[0]]
        for s in remaining_ind:
          if (s != remaining_ind[0]):
            training_data = torch.cat((training_data, x_train_folds[s]))
            training_label = torch.cat((training_label, y_train_folds[s]))
        classifier = KnnClassifier(training_data, training_label)
        accuracies.append(classifier.check_accuracy(validation_data, validation_label, k, quiet = True))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for i in k_to_accuracies:
      mean_acc = 0
      for s in k_to_accuracies[i]:
        mean_acc += s
      mean_acc /= len(k_to_accuracies[i])
