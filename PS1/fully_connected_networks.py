...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        for key in self.params.keys():
            self.params[key] = self.params[key].type(dtype).to(device)
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        out_1, cache_1 = Linear.forward(X, self.params["W1"], self.params["b1"])
        out_2, cache_2 = ReLU.forward(out_1)
        scores, cache_3 = Linear.forward(out_2, self.params["W2"], self.params["b2"])
        ...
        dx, grads["W2"], grads["b2"] = Linear.backward(dout, cache_3)
        dx = ReLU.backward(dx, cache_2)
        dx, grads["W1"], grads["b1"] = Linear.backward(dx, cache_1)

        for key in grads.keys():
          if key.startswith("W"):
            grads[key] += self.reg * 2 * self.params[key]
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for idx in range(1, len(hidden_dims)):
            self.params["W{}".format(idx+1)] = torch.randn(hidden_dims[idx - 1], hidden_dims[idx], dtype=dtype, device=device) * weight_scale
            self.params["b{}".format(idx+1)] = torch.zeros(hidden_dims[idx], dtype=dtype, device=device)

        self.params["W{}".format(self.num_layers)] = torch.randn(hidden_dims[-1], num_classes, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            output, caches_lin[str(i)] = Linear.forward(output, self.params[f"W{i}"], self.params[f"b{i}"])
            output, caches_relu[str(i)] = ReLU.forward(output)
            
        scores, caches_lin[str(self.num_layers)] = Linear.forward(output,
        ...
        dout, grads[f"W{self.num_layers}"], grads[f"b{self.num_layers}"] = \
          Linear.backward(dout, caches_lin[str(self.num_layers)]) 

        for i in range(self.num_layers - 1, 0, -1):
            dout = ReLU.backward(dout, caches_relu[str(i)])
            dout, grads[f"W{i}"], grads[f"b{i}"] = Linear.backward(dout, caches_lin[str(i)])
        
        #reg loss
        for key in self.params:
          if key.startswith("W"):
            grads[key] += self.reg * self.params[key]
            loss += self.reg * 0.5 * torch.sum(self.params[key] ** 2)
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, device=device, num_epochs=20, batch_size=32, lr_decay=0.99)
    solver.train()
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
