...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params = nn.ModuleDict({
          # NOTE: Ending Comma is important
          'lat_c3' : nn.Conv2d(dummy_out_shapes[0][1][1], self.out_channels, 1, stride = 1, padding = 0),
          'lat_c4' : nn.Conv2d(dummy_out_shapes[1][1][1], self.out_channels, 1, stride = 1, padding = 0),
          'lat_c5' : nn.Conv2d(dummy_out_shapes[2][1][1], self.out_channels, 1, stride = 1, padding = 0),

          "out_p3" : nn.Sequential(
          ...
          "out_p4" : nn.Conv2d(self.out_channels, self.out_channels, 3, stride = 1, padding = 1),
          "out_p5" : nn.Conv2d(self.out_channels, self.out_channels, 3, stride = 1, padding = 1)
        })
    ...
    def forward(self, images: torch.Tensor):
        ...
        p4 = p4 + F.interpolate(p5, p4.shape[2:], mode = "nearest")
        p3 = p3 + F.interpolate(p4, p3.shape[2:], mode = "nearest")
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # get the mesh grids
        xx, yy = torch.meshgrid(torch.arange(height), torch.arange(width))

        # Add the 0.5 shift to the meshgrid for finding the center of each box
        loc = torch.stack((xx + 0.5, yy + 0.5), dim=-1)
        loc = loc.type(dtype).to(device)
        location_coords[level_name] = (loc * level_stride).reshape(-1, 2)
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    keep = []
    # re-arrange the list to descending order
    ...
    # Select the highest-scoring box
        ...
        # Calculate IoU for the highest-scoring box with the remaining boxes
        tl = torch.max(boxes[i, :2], boxes[order[1:], :2])  # top-left
        br = torch.min(boxes[i, 2:], boxes[order[1:], 2:])  # bottom-right

        wh = (br - tl).clamp(min=0)  # width-height
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
          conv_cls = nn.Conv2d(in_channels, stem_channels[i], 3, stride=1, padding=1)
          conv_box = nn.Conv2d(in_channels, stem_channels[i], 3, stride=1, padding=1)
        ...
        self.pred_cls = nn.Conv2d(in_channels, num_classes, 3, stride = 1, padding = 1)  # Class prediction conv
        self.pred_box = nn.Conv2d(in_channels, 4, 3, stride = 1, padding = 1)  # Box regression conv
        self.pred_ctr = nn.Conv2d(in_channels, 1, 3, stride = 1, padding = 1)  # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key in feats_per_fpn_level.keys():
          cls_out = self.stem_cls(feats_per_fpn_level[key])
          box_out = self.stem_box(feats_per_fpn_level[key])
          #NOTE:!!!!
          #locations used meshgrid of "ij" indexing: (x, y) order -> (B, W*H, feat_dim) after flattening
          #while predictions are in (y, x) order due to image's default matrix representation  
          #so MUST permute (0, 3, 2, 1) instead of (0, 2, 3, 1) to get (B, feat_dim, W*H) after flattening
          class_logits[key] = self.pred_cls(cls_out).permute(0, 3, 2, 1)
          class_logits[key] = class_logits[key].contiguous().reshape(feats_per_fpn_level[key].size(0), -1, self.pred_cls.out_channels)
          boxreg_deltas[key] = self.pred_box(box_out).permute(0, 3, 2, 1)
          boxreg_deltas[key] = boxreg_deltas[key].contiguous().reshape(feats_per_fpn_level[key].size(0), -1, 4)
          centerness_logits[key] = self.pred_ctr(box_out).permute(0, 3, 2, 1)
          centerness_logits[key] = centerness_logits[key].contiguous().reshape(feats_per_fpn_level[key].size(0), -1, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    # restore to the unnormalized status
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    min_deltas = torch.min(deltas[:, [0, 2]], dim=-1)[0] * torch.min(deltas[:, [1, 3]], dim=-1)[0]
    max_deltas = torch.max(deltas[:, [0, 2]], dim=-1)[0] * torch.max(deltas[:, [1, 3]], dim=-1)[0]
    centerness = torch.sqrt(min_deltas / max_deltas)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(feats_shapes, strides, device=images.device)        
        ...
        for idx in range(len(images)):
            matched_gt_boxes.append(fcos_match_locations_to_gt(
                locations_per_fpn_level, strides, gt_boxes[idx]))
        ...
        for idx in range(len(images)):
            deltas = {}
            for key in matched_gt_boxes[idx].keys():
                deltas[key] = fcos_get_deltas_from_locations(
                    locations_per_fpn_level[key], matched_gt_boxes[idx][key], strides[key])
        ...
        #calc center ground truth
        centerness_gt = fcos_make_centerness_targets(matched_gt_deltas.reshape(-1, 4))

        #calc losses
        target_logits = matched_gt_boxes[:, :, 4].clamp(min=0)
        target_logits = F.one_hot(target_logits.long(), num_classes=self.num_classes)
        target_logits[matched_gt_boxes[:, :, 4] == -1] = 0 #suppress all classes to 0 for background
        target_logits = target_logits.to(torch.float32)

        loss_cls = sigmoid_focal_loss(pred_cls_logits, target_logits, reduction='none')        
        loss_box = 0.25 * F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction='none')
        loss_ctr = F.binary_cross_entropy_with_logits(  #do NOT use tensor.sigmoid()
            pred_ctr_logits.flatten(), centerness_gt, reduction='none'
            )
            
        loss_box[matched_gt_deltas < 0] = 0
        loss_ctr[centerness_gt < 0] = 0
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            #Debugging
            # print("level_locations.shape", level_locations.shape)
            # print("level_cls_logits.shape", level_cls_logits.shape)
            # print("level_deltas.shape", level_deltas.shape)
            # print("level_ctr_logits.shape", level_ctr_logits.shape)
            ...
            level_pred_scores = level_cls_logits.sigmoid_() * level_ctr_logits.sigmoid_()
            
            # Step 1:
            # Replace "PASS" statement with your code
            level_pred_classes = torch.argmax(level_pred_scores, dim=1)
            level_pred_scores, _ = torch.max(level_pred_scores, dim=1)
            # Step 2:
            # Replace "PASS" statement with your code
            ...
            level_cls_logits = level_cls_logits[mask]
            level_ctr_logits = level_ctr_logits[mask]
            ...
            # Replace "PASS" statement with your code
            level_pred_boxes[:, 0::2].clamp_(min=0, max=images.shape[3])
