...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.matmul(features_reshape, features_reshape.transpose(1, 2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer in enumerate(style_layers):
      ...
      current_gram_matrix = gram_matrix(current_features)
      target_gram_matrix = style_targets[i]
      layer_loss = style_weights[i] * torch.sum((current_gram_matrix - target_gram_matrix) ** 2)
      loss += layer_loss
...
def tv_loss(img, tv_weight):
    ...
    h_diff_loss = torch.sum(h_diff ** 2)
    w_diff_loss = torch.sum(w_diff ** 2)

    loss = tv_weight * (h_diff_loss + w_diff_loss)
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  # this problem.                                                              #
  ##############################################################################
  N, _, C, H, W = features.shape
  R = masks.shape[1]
  guided_gram = torch.zeros(N, R, C, C, device=features.device)

  for i in range(R):
      mask = masks[:, i, :, :].repeat(1, C, 1, 1) #unsqueeze channel dim
      masked_features = features[:, i, ...] * mask 
      features_reshape = masked_features.view(N, C, -1)
      gram = torch.matmul(features_reshape, features_reshape.transpose(1, 2))

      if normalize:
        #   non_zero_elements = torch.sum(mask).item()  # Calculate the number of non-zero elements
          gram /= H * W * C  # Updated normalization factor
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    for i, layer in enumerate(style_layers):
          current_features = feats[layer]
          current_guided_gram_matrix = guided_gram_matrix(current_features, content_masks[layer])
          target_guided_gram_matrix = style_targets[i]
          layer_loss = style_weights[i] * torch.sum((current_guided_gram_matrix - target_guided_gram_matrix) ** 2)
